...
class TestDataset(object):
    def __init__(self, ann_file, dataroot,final_test, test_mode=False, **kwargs):
        super().__init__()
                ...
                if not line.strip():
                    ...
                    if not self.final_test:
                        ...
                        data_infos.append(dict(feature_path=feature_path, label_path=label_path,
                                               instance_count_path=instance_count_path,
                                               instance_IR_drop_path=instance_IR_drop_path,
                                               instance_name_path=instance_name_path))
                    else:
                        ...
                        if self.dataroot is not None:
                            ...
                            instance_count_path = os.path.join(self.dataroot, instance_count)
                            instance_name_path = os.path.join(self.dataroot, instance_name)
                        data_infos.append(dict(feature_path=feature_path,
                                               instance_count_path=instance_count_path,
                                               instance_name_path=instance_name_path))
        return data_infos
    ...
    def prepare_data(self, idx):
        if not self.final_test:
            ...
            return feature, label, results['instance_count_path'], results['instance_IR_drop_path'], results[
                'instance_name_path']
        else:
            null1=0
            null2=0
            results = copy.deepcopy(self.data_infos[idx])
            feature = np.load(results['feature_path']).transpose(2, 0, 1).astype(np.float32)
            return feature,null1, results['instance_count_path'],null2, results['instance_name_path']
