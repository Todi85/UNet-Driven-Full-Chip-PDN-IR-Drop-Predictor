...
@masked_loss
def l1_l2_loss(pred, target):
    #0.7和0.3也不错
    return 0.7*F.l1_loss(pred, target, reduction='none')+0.3*F.mse_loss(pred, target, reduction='none')
...
class L1L2(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        return self.loss_weight * l1_l2_loss( pred,
            target,
            weight,
            reduction=self.reduction,
            sample_wise=self.sample_wise)