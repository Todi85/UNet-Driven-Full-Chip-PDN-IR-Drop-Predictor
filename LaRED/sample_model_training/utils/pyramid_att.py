...
import sample_model_training.utils.pyramid_tool as pt



class PyramidAttention(nn.Module):
    def __init__(self, level=5, res_scale=1, channel=256, reduction=2, ksize=3, stride=1, softmax_scale=10, average=True,
                 conv=pt.default_conv):
        ...
        self.conv_match_L_base = pt.BasicBlock(conv, channel, channel // reduction, 1, bn=False, act=nn.PReLU())
        self.conv_match = pt.BasicBlock(conv, channel, channel // reduction, 1, bn=False, act=nn.PReLU())
        self.conv_assembly = pt.BasicBlock(conv, channel, channel, 1, bn=False, act=nn.PReLU())
    ...
    def forward(self, input):
        ...
        for i in range(len(self.scale)):
            ...
            # sampling
            raw_w_i = pt.extract_image_patches(base, ksizes=[kernel, kernel],
            ...
            # sampling
            w_i = pt.extract_image_patches(ref_i, ksizes=[self.ksize, self.ksize],
                                        strides=[self.stride, self.stride],
                                        rates=[1, 1],
                                        padding='same')
        ...
        for idx, xi in enumerate(input_groups):
            ...
            # normalize
            max_wi = torch.max(torch.sqrt(pt.reduce_sum(torch.pow(wi, 2),
                                                     axis=[1, 2, 3],
                                                     keepdim=True)),
                               self.escape_NaN)
            wi_normed = wi / max_wi
            # matching
            xi = pt.same_padding(xi, [self.ksize, self.ksize], [1, 1], [1, 1])  # xi: 1*c*H*W
