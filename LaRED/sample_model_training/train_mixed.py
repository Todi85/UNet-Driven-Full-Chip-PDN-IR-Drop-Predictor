...
def train():
    ...
    for feature, label, _ in dataset:
        ...
        if iter_num % print_freq == 0:
            end_time = time.time()
            logger.info("===> Iters[{}]({}/{}): Loss: {:.4f} Time: {:.1f}".format(iter_num, iter_num, arg_dict['max_iters'],epoch_loss / print_freq,(end_time-start_time)/60))
            writer.add_scalar('Loss/training loss', epoch_loss / print_freq, iter_num)
            epoch_loss = 0
            start_time=end_time
        if iter_num % save_freq == 0:
            checkpoint(logger, model, iter_num, log_dir)
        if iter_num == int(arg_dict['max_iters']):
            break
